        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            {        
                case SignInStatus.Success:
                    ...
                    if (UserManager.IsInRole(user.Id,"SuperAdmin") || UserManager.IsInRole(user.Id,"Manager"))
                    {
                        return RedirectToAction("Teds", "Home");
                    }
                    else if (UserManager.IsInRole(user.Id, "SepsR"))
                    {
                        return RedirectToAction("SEPS_DB", "Home");
                    }
                    else if (UserManager.IsInRole(user.Id, "TedsR"))
                    {
                        return RedirectToAction("Teds", "Home");
                    }
       ...
       // [AllowAnonymous]
        ...
        { 
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ApplicationUser newuser = context.Users.Where(u => u.UserName.Equals(model.Email, StringComparison.CurrentCultureIgnoreCase)).FirstOrDefault();
        ...
        public ActionResult AssignRole()
        {
            ...
            {
                ViewBag.Roles = context.Roles.Where(a => a.Name != "SuperAdmin").Select(r => new SelectListItem { Value = r.Name, Text = r.Name }).ToList();
                ViewBag.Emails = 
                    ...
                    // {
                    //     Email = user.Email,
                    //     Role = (from userRoles in user.Roles join role in context.Roles on userRoles.RoleId equals role.Id
                    //             select role.Name)
                    // }).Where(f => !f.Role.Equals("SuperAdmin")).Select(u => new SelectListItem { Value = u.Email, Text = u.Email }).ToList();



                context.Users.Select(u => new SelectListItem { Value = u.Email, Text = u.Email }).ToList();
            }
            else
            {
                ViewBag.Roles = context.Roles.Select(r => new SelectListItem { Value = r.Name, Text = r.Name }).ToList();
                ViewBag.Emails = context.Users.Select(u => new SelectListItem { Value = u.Email, Text = u.Email }).ToList();
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult AssignRole(string submit)
        {
            ...
            if (submit == "Add Role")
            {
                try
                {
                    string user = Request.Form["Emails"];
                    string role = Request.Form["RoleName"];

                    ApplicationUser ExistingUser = context.Users.Where(e => e.Email.Equals(user, StringComparison.CurrentCultureIgnoreCase)).FirstOrDefault();
                    var userManager = new UserManager<ApplicationUser>(new UserStore<ApplicationUser>(context));
            ...
            else if (submit == "Delete Role")
            {
                try
                {
                    ...
                    if (userManager.IsInRole(ExistingUser.Id, role))
                    {
                        userManager.RemoveFromRole(ExistingUser.Id, role);
                    }
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user == null )//|| !(await UserManager.IsEmailConfirmedAsync(user.Id)))
                {
                    ...
                    ModelState.AddModelError("", "No existe email ingresado.");
                    return View(model);
                } 
